...
export default function BackDrop() {
  const { TotalAmount , removeAll } = useContext(ctx)
  const {isCheckOutOpen,isCheckOutOpenHandler} = useContext(CheckOutContext)
  const [user , setUser] = useState({})
  const [displayBtn , setDisplayBtn] = useState(true)
  const history = useHistory();
  const notifyDone = () => toast.success('Your Payment is Done');
  const notifyInfo = () => toast.info(' To Get Out From This menu you Should double-Click on BackDrop  ');

  useEffect(_=>notifyInfo(),[])
...
<section className="z-20 flex flex-col justify-center antialiased bg-gray-200 text-gray-600 min-h-screen p-4">
        ...
        <div className="max-w-[360px] mx-auto">
            <div className="bg-white shadow-lg rounded-lg mt-9">
                <header className="text-center px-5 pb-5">
                    ...
                    <div className="text-sm font-medium text-gray-500">please enter your visa information </div>
                </header>
                <div className="bg-gray-100 text-center px-5 py-6">
                    <div className="text-sm mb-6"><strong className="font-semibold">${TotalAmount}</strong></div>
